...
- [x] 2.5 Solo si la tarea 1.3 lo pidió, anular el alto fijo en `base.css` acotándolo a `#tablaLineas .table-row-cell`, nunca sobre `.table-view`. (1.3 decidió que no hacía falta: el 34 px era mínimo.)
...
- [x] 3.4 En la aplicación: escribir una descripción larga, confirmar con ENTER y comprobar que la fila crece, que se lee entera y que el foco salta a Precio. Comprobar también que al escribir se encadenan cantidad, descripción, precio y total como hasta ahora.
- [x] 3.5 Maximizar la ventana y comprobar que la columna de descripción se ensancha y que esa misma línea pasa a ocupar menos alto.
- [x] 3.6 Con una factura de muchas líneas, comprobar que el desglose de totales del pie sigue visible y que solo se desplaza la tabla.
- [x] 3.7 Generar el PDF de esa factura y comprobar que la descripción impresa coincide con la que se ve en pantalla.
- [x] 3.8 Abrir Clientes, Histórico y la matriz de IVA del Editor y comprobar que sus filas conservan el alto de siempre.
- [x] 3.9 Repetir 3.4 y 3.5 con un tema claro y con uno oscuro, comprobando que el texto de la descripción se lee en ambos.
...
- [x] 4.1 En `CeldaEditable`, cambiar el tipo del campo `editor` de `TextField` a `TextInputControl` y crearlo en un método que las subclases puedan sobrescribir. No tocar el manejador de ENTER y ESCAPE, ni el listener de foco, ni `commitYAvanzar`, ni `commitSolo`, ni `commitValor`. Ver `design.md - D5`.
- [x] 4.2 Comprobar que `CeldaCantidad`, `CeldaPrecio` y `CeldaTotal` siguen sin cambios y con su `TextField`.
- [x] 4.3 En `CeldaDescripcion`, crear un `TextArea` con `setWrapText(true)` y `setPrefRowCount(1)`, y atar su `prefHeight` al alto del nodo de texto interno (`lookup(".text")`) en cuanto el control tenga skin. Ver `design.md - D5`.
- [x] 4.4 Hacer que `computePrefHeight` de `CeldaDescripcion` mida el editor cuando la celda está en edición y la etiqueta cuando no. **Medir** con un test temporal, como en la tarea 1, que la fila crece al meter texto largo en el editor abierto, y que el `TextArea` no saca barra de desplazamiento. Si la `VirtualFlow` no recalcula el alto, pedir el recálculo de esa fila; **nunca** con `tablaLineas.refresh()`, que tira el editor. Borrar el test al terminar.
- [x] 4.5 En `editarCeldaSegura`, buscar el editor por `.text-field` y por `.text-area`, y tratarlo como `TextInputControl`. Adaptar igual las dos búsquedas de `EditorFlujoTecladoTest` (líneas 118 y 127), sin cambiar lo que el test comprueba. Ver `design.md - D6`.
- [x] 4.6 `mvn test` en verde y, en la aplicación: escribir una descripción larga y ver que salta de renglón y que la fila crece **mientras** se teclea; confirmar con ENTER y comprobar que el foco sigue yendo a Precio; comprobar que ENTER no deja ningún salto de línea dentro de la descripción, que ESCAPE sigue cancelando y qué hace la tecla TAB dentro del campo.
- [x] 4.7 Repetir las comprobaciones 3.4 a 3.9, que quedaron pendientes, ya con el editor nuevo.
...
- [x] 5.6 Comprobar a mano lo mismo: pinchar en una descripción corta y ver que la fila no da ningún salto; escribir sin parar y ver que el campo enseña todo lo escrito en todo momento, sin barra y sin tener que pulsar ENTER.
