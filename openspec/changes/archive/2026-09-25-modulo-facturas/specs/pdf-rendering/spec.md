...
**Reason**: Se rehace sin versiones con otro nombre, porque uno de sus escenarios nombraba las versiones en el título.
**Migration**: Sustituido por «Exportación de facturas a PDF», con el mismo contenido.
...
- Tabla de líneas estilo hoja de cálculo: Cant / Descripción / Precio / IVA % / Total. La tabla SHALL conservar sus bordes laterales y las divisiones verticales entre columnas, la raya superior de la fila de cabecera de columnas y la raya que separa esa cabecera de la primera línea. Entre una línea y la siguiente SHALL NOT dibujarse ninguna raya horizontal; el rayado alterno de fondo SHALL mantenerse para distinguirlas. La tabla SHALL quedar cerrada por abajo con una raya horizontal: la del marco de columnas antes de los suplidos o de los totales y, cuando la tabla se corte al final de una página, una raya en el borde inferior del trozo cortado. Las celdas de una línea SHALL usar un interlineado de 1 pt más que su tamaño de letra, de modo que las descripciones de varias líneas respiren. El Total por línea SHALL incluir el IVA (base × (1 + IVA%)); las líneas exentas SHALL mostrar su importe sin IVA. La descripción SHALL mostrarse siempre en un único estilo, aunque ocupe varias líneas. El texto de la tabla SHALL ir en negro. La fila de cabecera de columnas SHALL repetirse al principio de cada página que continúe la tabla.
...
El resto se mantiene como estaba: descripciones largas ajustadas automáticamente, importes en formato español y sin símbolo de moneda, que aparece una sola vez en la banda `TOTAL`, fechas formato español (`11/08/2026`), marca `ANULADA` destacada en facturas anuladas, correspondencia exacta con la factura exportada, uso de la configuración actual de empresa/logo/cabecera/pie legal, documentos independientes, estructura `Facturas/AAAA/SERIE/` y nombre `CODIGO-CORRELATIVO-MES.pdf`. El logo SHALL dibujarse siempre dentro de una caja fija de 240 × 120 pt respetando su proporción, sin que su tamaño ni su posición sean configurables, de modo que nunca invada el bloque FACTURA ni comprima la columna de datos de empresa. El logo SHALL escalarse sin recortarse ni deformarse hasta tocar el borde de la caja, y SHALL colocarse pegado a la esquina superior izquierda de la caja, de modo que su borde superior quede a la altura del nombre de la empresa sea cual sea su forma. En modo logo, los datos de empresa SHALL arrancar siempre en la misma posición, a 14 pt del borde derecho de la caja, con independencia del ancho real del logo. El color de acento SHALL tomarse de la preferencia `color_pdf`, con valor por defecto arena Alcazaba (`#B08D57`) si no está configurada.
...
- **THEN** el pie se compone a 5,5 pt dentro de su recuadro, sin que el texto se salga
...
#### Scenario: Líneas sin rayas entre artículos
- **WHEN** el usuario exporta una factura de varias líneas
- **THEN** entre una línea y la siguiente no aparece ninguna raya horizontal, y las líneas se distinguen por el rayado alterno de fondo
- **AND** se mantienen la raya superior de la cabecera de columnas, la raya bajo `CANT. / DESCRIPCIÓN / PRECIO / IVA % / TOTAL`, las divisiones verticales y la raya que cierra la tabla antes de los suplidos o de los totales

#### Scenario: La tabla se cierra al cambiar de página
- **WHEN** el usuario exporta una factura cuya tabla de líneas continúa en la página siguiente
- **THEN** en la página cortada la tabla termina con una raya horizontal en su borde inferior
- **AND** en la última página no aparece una raya entre la última línea y el marco de columnas

#### Scenario: Texto un punto mayor
- **WHEN** el usuario exporta cualquier factura
- **THEN** cada texto del documento se compone 1 pt mayor que antes de este cambio, incluidos la cabecera, las tarjetas, la tabla de líneas, los suplidos, los totales, las observaciones y `Página X de Y`
- **AND** el pie legal se compone a 5,5 pt
- **AND** la descripción de varias líneas usa un interlineado de 11 pt

#### Scenario: Sin raya bajo la última línea en una factura de una página
- **WHEN** el usuario exporta una factura de tres líneas que cabe en una página
- **THEN** bajo la tercera línea no aparece ninguna raya horizontal y el marco de columnas continúa hasta su raya de cierre

#### Scenario: Posición fija del logo y de los datos de empresa
- **WHEN** el usuario exporta dos facturas en modo logo, una con un logo cuadrado y otra con un logo apaisado
- **THEN** en las dos los datos de empresa arrancan en la misma posición, a 14 pt del borde derecho de la caja del logo
- **AND** en las dos el borde superior del logo queda arriba de su caja, a la altura del nombre de la empresa
