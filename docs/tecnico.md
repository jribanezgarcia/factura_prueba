...
flowchart TD
    ...
    B["🖥️ Pantallas<br/><b>ui/*Controller + *.fxml</b>"] -->|"servicios.factura, servicios.clientes…"| C
    C["📦 Servicios<br/><small>crea y reparte todo al arrancar</small>"] --> D
    D["🧠 Reglas<br/><b>service/*Service</b>"] --> E
    E["🗄️ Consultas SQL<br/><b>repository/*Repository</b>"] --> F
...
|---|---|---|
| 🖥️ **Pantallas** (`ui`) | Leer lo que escribe el usuario, llamar a un servicio y mostrar el resultado | Consultas SQL ni reglas de negocio |
| 📦 **Servicios** | Crear una vez todos los servicios y repositorios y entregárselos a las pantallas | Lógica |
| 🧠 **Reglas** (`service`) | Decidir: número repetido, totales, qué pasos y en qué orden, transacciones | Saber qué base de datos hay debajo |
| 🗄️ **Consultas** (`repository`) | `SELECT`, `INSERT`, `UPDATE`, `DELETE` de su tabla | Decidir si algo está permitido |
| 🔌 **Database** (`db`) | Abrir la conexión, crear las tablas, `commit` y `rollback` | Nada más |

> [!TIP]
> **💡 Concepto: repositorio = DAO.** Un *repository* es lo que en clase llamamos **DAO** (*Data Access Object*): la clase que sabe hablar con una tabla. Si mañana se cambia SQLite por otra base de datos, solo se tocan los repositorios y `Database`.
...
> [!TIP]
> **💡 Concepto: inyección por constructor.** `Servicios` hace los `new` una sola vez y pasa a cada clase lo que necesita por su constructor, por ejemplo `new FacturaService(facturaRepository, …, clock)`. Así se ve de un vistazo de qué depende cada clase, y en los tests se le puede pasar otra cosa (una fecha fija, por ejemplo).
...
├── 🚀 Main, Launcher            → punto de entrada JavaFX
...
├── 🖥️ ui/                       → controladores, FXML, navegación, diálogos y temas
├── 🧠 service/                  → reglas de negocio y el contenedor Servicios
├── 🗄️ repository/               → un DAO por tabla, más CopiaRepository para las copias
├── 🔌 db/                       → conexión, migraciones, DatosException y datos de demostración
├── 🧩 model/                    → clases de datos: Cliente, Factura, FacturaVersion, LineaFactura…
├── 📄 pdf/                      → construcción y dibujo del PDF
└── 🛠️ util/                     → formatos y validadores de NIF, código postal y email
```
...
2. **El servicio** comprueba que el número no esté ocupado (`NumeroService`), calcula los totales (`CalculoService`) y abre una transacción.
...
> [!TIP]
> **💡 Concepto: excepción no comprobada.** Los repositorios convierten `SQLException` en `DatosException`, que hereda de `RuntimeException`. Así las pantallas y los servicios no tienen que declarar `throws SQLException` y no dependen de que haya una base de datos SQL debajo.
...
| **Capas service + repository** | Reglas y SQL en la misma clase | Las reglas se prueban y reutilizan sin tocar SQL, y cambiar de base de datos solo afecta a los repositorios |
