...
public class Numeracion {
    ...
    public Numeracion(SerieDAO serieDAO, NumeroDisponibleDAO numeroDisponibleDAO, Clock clock) {
     ...
     */
    public int siguienteCorrelativo(Serie serie) {
     ...
     */
    public int siguienteCorrelativo(Serie serie, LocalDate fecha) {
        ...
        if (serie.isReutilizarAnulados()) {
            Set<Integer> anuladas = serieDAO.correlativosAnuladas(serie.getId(), anio);
            Set<Integer> activos = serieDAO.correlativosActivos(serie.getId(), anio);
        ...
        int siguiente = serieDAO.getSiguiente(serie.getId(), anio);
        for (int ocupado : serieDAO.correlativosActivos(serie.getId(), anio)) {
     ...
     */
    public List<Integer> huecosDisponibles(Serie serie, LocalDate fecha) {
        int anio = fecha != null ? fecha.getYear() : LocalDate.now(clock).getYear();
        Set<Integer> activos = serieDAO.correlativosActivos(serie.getId(), anio);
        List<Integer> todos = numeroDisponibleDAO.listar(serie.getId(), anio);
    ...
    public List<Integer> proponerNumeros(Serie serie, int anio, int cantidad, boolean usarHuecos)
        ...
        Set<Integer> ocupados = serieDAO.correlativosActivos(serie.getId(), anio);
    ...
    public boolean correlativoOcupadoPorActiva(Serie serie, int correlativo) {
        return correlativoOcupadoPorActiva(serie, correlativo, LocalDate.now(clock));
    }

    public boolean correlativoOcupadoPorActiva(Serie serie, int correlativo, LocalDate fecha) {
        int anio = fecha != null ? fecha.getYear() : LocalDate.now(clock).getYear();
        return serieDAO.correlativosActivos(serie.getId(), anio).contains(correlativo);
    }
