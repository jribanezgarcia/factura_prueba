...
public class HistorialDAO {
    ...
    public List<FilaHistorial> buscar(FiltrosHistorial f) {
        try {
            ...
            List<FilaHistorial> lista = new ArrayList<>();
            try (PreparedStatement ps = Conexion.establecerConexion().prepareStatement(sql.toString())) {
                for (int i = 0; i < params.size(); i++) {
                    ps.setObject(i + 1, params.get(i));
                }
                try (ResultSet rs = ps.executeQuery()) {
                    while (rs.next()) {
                        FilaHistorial fila = map(rs);
    ...
    private boolean cumpleImporte(FilaHistorial fila, FiltrosHistorial f) {
    ...
    private FilaHistorial map(ResultSet rs) throws SQLException {
